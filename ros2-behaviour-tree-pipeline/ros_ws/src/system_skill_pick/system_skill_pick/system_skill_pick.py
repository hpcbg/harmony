...
class CheckBottleDetected(py_trees.behaviour.Behaviour):
    def __init__(self, name):
        super().__init__(name)
        ...
        self.blackboard.detected_bottle = {'found': False}

    def setup(self, **kwargs):
        self.node = kwargs['node']
        self.subscriber = self.node.create_subscription(
            String, '/bottle_detector/detected_bottle_json', self.callback, 10
        )

    def callback(self, msg):
        self.blackboard.detected_bottle = json.loads(msg.data)

    def update(self):
        return py_trees.common.Status.RUNNING


class MoveToGraspPose(py_trees.behaviour.Behaviour):
    def __init__(self, name):
        super().__init__(name)
        self.node = None
        ...
        self.blackboard = py_trees.blackboard.Client(name=name)
        self.blackboard.register_key(
            key='detected_bottle', access=py_trees.common.Access.READ)
    ...
    def update(self):
        ...
        goal_msg.pose_json = json.dumps(
            self.blackboard.detected_bottle['pose'])
        self.node.get_logger().info(
            f'Send move action to the position of the detected bottle: {self.blackboard.detected_bottle['pose']}.')
...
class ObjectAvailable(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        if self.blackboard.detected_bottle['found']:
            self.logger.info(
                f'Bottle detected at position {self.blackboard.detected_bottle['pose']}')
            return py_trees.common.Status.SUCCESS
        else:
            self.logger.info('Waiting for a bottle detection...')
            return py_trees.common.Status.RUNNING
...
class PickNode(Node):
    def __init__(self):
        ...
        system_state.add_children([
            ...
            CheckBottleDetected('Bottle Detector Status')
        ])

        move_to_grasp_seq = py_trees.composites.Sequence(
            'Move to Grasp Position', memory=True)
        move_to_grasp_seq.add_children([
            ObjectAvailable('Bottle Detected?'),
            RobotAvailable('Robot Stationary?'),
            ReleaseGripper('Release Gripper'),
            WaitNode('Wait 1 sec', wait_time=1.0),
            RobotAvailable('Robot Stationary?'),
            MoveToGraspPose('Send Grasp Position'),
            WaitNode('Wait 1 sec', wait_time=1.0)
        ])

        grasp_seq = py_trees.composites.Sequence(
            'Grasp', memory=True)
        grasp_seq.add_children(
            [RobotAvailable('Robot At Grasp Position?'),
