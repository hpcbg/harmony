#!/usr/bin/env bash
# Gesture command publisher — two backends, selected via GESTURE_BACKEND:
#
...
#
#   GESTURE_BACKEND=ros2                   DDS-native experimental path: publish
#                                         std_msgs/String on /user_inputs/gesture_command.
#                                         Orion-LD (-wip dds) maps that ROS 2/DDS topic
#                                         to NGSI-LD, so no /v2 calls and no HTTP 501.
#                                             GESTURE_BACKEND=ros2 ./run.sh
#
# No-camera self-test (any backend): set TEST_GESTURE to publish without a camera,
# OpenCV or MediaPipe, e.g.
...
BACKEND="${GESTURE_BACKEND:-fiware_v2}"
...
exec "$PY" gesture-commands-fiware.py --no-gui
