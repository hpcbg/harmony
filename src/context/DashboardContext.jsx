...
import WidgetExistsModal from "../components/modals/WidgetExistsModal";
...
export const DashboardProvider = ({ children }) => {
  ...
  const [widgetLibrary, setWidgetLibrary] = useLocalStorage(
  ...
  const addWidget = useCallback(
    (widgetData, pageId = currentPage) => {
      ...
      setWidgetLibrary((prev) => [...prev, widget]);
          ...
          prev.map((page) =>
            page.id === pageId
              ? {
                  ...
                  widgets: [
                    ...page.widgets,
                    { id: widget.id, x: 50, y: 50, width: 300, height: 200 },
                  ],
                }
              : page
          )
    ...
    [currentPage, setWidgetLibrary, setPages]
  );
  ...
  const deleteWidget = useCallback(
    ...
    [setPages, setWidgetLibrary, openConfirm]
  );

  const removeWidgetFromPage = useCallback(
    (widgetId, pageId = currentPage) => {
      openConfirm(
        "Are you sure you want to remove the widget from the page?",
        () => {
          setPages((prev) =>
            prev.map((page) =>
              page.id === pageId
  ...
  const moveWidget = useCallback(
    (widgetId, position, pageId = currentPage) => {
      setPages((prev) =>
        prev.map((page) =>
          page.id === pageId
            ? {
  ...
  const addFromLibrary = useCallback(
    (libraryWidget, pageId = currentPage) => {
      ...
      setPages((prev) => {
        return prev.map((page) => {
          ...
          if (page.widgets.some((w) => w.id === libraryWidget.id)) {
            alreadyExists = true;
          ...
          return {
            ...
            widgets: [
              ...page.widgets,
              { id: libraryWidget.id, x: 50, y: 50, width: 300, height: 200 },
            ],
          };
        });
      });
    ...
    [currentPage, setPages, setShowExistsModal]
  );

  // Export/Import operations
  const exportConfig = useCallback(() => {
    const config = { pages, widgetLibrary };
